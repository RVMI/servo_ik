...
class SimIiwa7DemoNode(object):
  def __init__(self):
    ...
    rate = rospy.Rate(100)
    cartesian_pose_pub = rospy.Publisher('cartesian_pose', PoseStamped, queue_size = 1)

    while not rospy.is_shutdown():
      t = rospy.Time.now().to_sec()
      cartesian_pose = PoseStamped()
      cartesian_pose.pose.position.x = 0.5 + 0.1 * cos(2 * pi * t)
      cartesian_pose.pose.position.y = 0.1 * sin(2 * pi * t)
